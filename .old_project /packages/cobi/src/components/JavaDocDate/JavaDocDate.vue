<template>
  <div class="java-doc-date">
    <el-drawer z-index="6003" append-to-body size="40%" title="Documentation" v-model="dialogVisible" direction="rtl">
      <h4>Date and Time Patterns</h4>
      <p>
        Date and time formats are specified by <em>date and time pattern</em>
        strings. Within date and time pattern strings, unquoted letters from
        <code>'A'</code> to <code>'Z'</code> and from <code>'a'</code> to <code>'z'</code> are interpreted as pattern letters representing the components of a date or time string. Text can be quoted using single quotes (<code>'</code>) to avoid interpretation. <code>"''"</code> represents a single quote. All other characters are not interpreted; they're simply copied into the output string during formatting or matched against the input string during parsing.
      </p>
      <p>
        The following pattern letters are defined (all other characters from
        <code>'A'</code> to <code>'Z'</code> and from <code>'a'</code> to <code>'z'</code> are reserved):
      </p>
      <blockquote>
      ...
      <ul>
        <li>
          <strong><a name="text">Text:</a></strong> For formatting, if the number of pattern letters is 4 or more, the full form is used; otherwise a short or abbreviated form is used if available. For parsing, both forms are accepted, independent of the number of pattern letters.<br /><br />
        </li>
        <li>
          <strong><a name="number">Number:</a></strong> For formatting, the number of pattern letters is the minimum number of digits, and shorter numbers are zero-padded to this amount. For parsing, the number of pattern letters is ignored unless it's needed to separate two adjacent fields.<br /><br />
        </li>
        <li>
          <strong><a name="year">Year:</a></strong> If the formatter's <a href="../../java/text/DateFormat.html#getCalendar()"><code>Calendar</code></a> is the Gregorian calendar, the following rules are applied.<br />
          <ul>
            ...
            <li>
              For parsing with the abbreviated year pattern ("y" or "yy"), <code>SimpleDateFormat</code> must interpret the abbreviated year relative to some century. It does this by adjusting dates to be within 80 years before and 20 years after the time the <code>SimpleDateFormat</code>
          ...
          Otherwise, calendar system specific forms are applied. For both formatting and parsing, if the number of pattern letters is 4 or more, a calendar specific <a href="../../java/util/Calendar.html#LONG">long form</a> is used. Otherwise, a calendar specific <a href="../../java/util/Calendar.html#SHORT">short or abbreviated form</a> is used.<br />
          <br />
          If week year <code>'Y'</code> is specified and the <a href="../../java/text/DateFormat.html#getCalendar()">calendar</a> doesn't support any <a href="../util/GregorianCalendar.html#week_year"> week years</a>, the calendar year (<code>'y'</code>) is used instead. The support of week years can be tested with a call to <a href="../../java/text/DateFormat.html#getCalendar()"><code>getCalendar()</code></a
          >.<a href="../../java/util/Calendar.html#isWeekDateSupported()"><code>isWeekDateSupported()</code></a
          >.<br /><br />
        </li>
        <li>
          <strong><a name="month">Month:</a></strong> If the number of pattern letters is 3 or more, the month is interpreted as <a href="#text">text</a>; otherwise, it is interpreted as a <a href="#number">number</a>.<br /><br />
        </li>
        <li>
          <strong><a name="timezone">General time zone:</a></strong> Time zones are interpreted as <a href="#text">text</a> if they have names. For time zones representing a GMT offset value, the following syntax is used:
     ...
     <i>Digit:</i> one of
          ...
          <p>For parsing, <a href="#rfc822timezone">RFC 822 time zones</a> are also accepted.<br /><br /></p>
        </li>
        <li>
     ...
     <i>TwoDigitHours:</i>
          ...
          <p>For parsing, <a href="#timezone">general time zones</a> are also accepted.</p>
        </li>
        <li>
     ...
     <i>ThreeLetterISO8601TimeZone:</i>
          ...
          <p>For formatting, if the offset value from GMT is 0, <code>"Z"</code> is produced. If the number of pattern letters is 1, any fraction of an hour is ignored. For example, if the pattern is <code>"X"</code> and the time zone is <code>"GMT+05:30"</code>, <code>"+05"</code> is produced.</p>
          <p>For parsing, <code>"Z"</code> is parsed as the UTC time zone designator. <a href="#timezone">General time zones</a> are <em>not</em> accepted.</p>
          <p>
            If the number of pattern letters is 4 or more, <a href="../../java/lang/IllegalArgumentException.html" title="class in java.lang"><code>IllegalArgumentException</code></a> is thrown when constructing a <code>SimpleDateFormat</code> or <a href="../../java/text/SimpleDateFormat.html#applyPattern(java.lang.String)">applying a pattern</a>.
          </p>
        </li>
      </ul>
