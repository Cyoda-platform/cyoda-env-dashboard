<template>
  <div class="body">
    ...
    <el-collapse-transition :key="value" v-if="isAvailableChild || !isWasRequest">
...
import { Column, ElementUiOption, Entity } from "../../../../../types/types";
...
const options = computed(() => {
  const options: ElementUiOption[] = [];
  for (let i = 0; i < props.column.listSize; i++) {
    options.push({
      label: `Option ${i + 1}`,
      value: i
    });
