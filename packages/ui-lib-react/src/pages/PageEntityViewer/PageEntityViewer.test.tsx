...
import * as entitiesApi from '@cyoda/http-api-react';

// Mock dependencies
vi.mock('../../stores/entityViewerStore');
vi.mock('@cyoda/http-api-react/stores', () => ({
...
vi.mock('@cyoda/http-api-react/utils', () => ({
  HelperEntities: { getOptionsFromData: (data: any[]) => data.map((d: any) => ({ value: d.name || d, label: d.name || d })) },
  HelperFeatureFlags: { isCyodaCloud: () => false },
  eventBus: { on: vi.fn(), off: vi.fn(), emit: vi.fn() },
}));
vi.mock('@cyoda/http-api-react/utils/HelperEntities', () => ({
  default: { getOptionsFromData: (data: any[]) => data.map((d: any) => ({ value: d.name || d, label: d.name || d })) },
}));
...
vi.mock('../../components/ConfigEditorStreamGrid', () => ({
  ConfigEditorStreamGrid: vi.fn(() => <div data-testid="stream-grid">StreamGrid</div>),
}));
...
describe('PageEntityViewer', () => {
  ...
  describe('Event Handling', () => {
    it('should listen for entityInfo:update events', async () => {
      ...
      const { eventBus } = await import('@cyoda/http-api-react/utils');
