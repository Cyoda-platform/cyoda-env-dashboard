...
const ReportConfigs: React.FC<ReportConfigsProps> = ({ onResetState }) => {
  ...
  const createReportMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      refetch();
      navigate(`/reporting/report-editor/${data.content}?isNew=true`);
    },
    onError: (error: any) => {
  ...
  const cloneReportMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      refetch();
      navigate(`/reporting/report-editor/${data.content}`);
    },
    onError: (error: any) => {
  ...
  const handleEditReport = useCallback((row: ReportConfigRow) => {
    navigate(`/reporting/report-editor/${encodeURIComponent(row.id)}`);
  }, [navigate]);
