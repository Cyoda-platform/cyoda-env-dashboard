...
const HistoryReportsTab: React.FC<{ onResetState: () => void }> = ({ onResetState }) => {
  ...
  return (
    <div className="history-view-reports">
      ...
      {/* Report Data Rows - shown when a report is selected */}
      {reportDefinition && configDefinition && Object.keys(configDefinition).length > 0 && (
        <div className="report-table">
          <div className="wrap-table full">
            <span className="label">Report Data</span>
            <ReportTableRows
              tableLinkRows={`/platform-api/reporting/report/${reportDefinition.id}/rows`}
              lazyLoading={settings.lazyLoading}
              configDefinition={configDefinition}
              reportDefinitionId={reportDefinition.id}
              onShowColumnDetail={(data) => columnCollectionsDialogRef.current?.showDetail(data)}
