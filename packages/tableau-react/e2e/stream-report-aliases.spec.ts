/**
 * E2E tests for Aliases in Stream Reports
 * Tests alias functionality in the Stream Report Editor
 */
...
test.describe('Stream Report Editor - Aliases Tab', () => {
  ...
  test('should display Stream Report Editor page', async ({ page }) => {
    ...
    await page.waitForSelector('.ant-tabs, h1, h2');
  ...
  test('should display Model tab for aliases', async ({ page }) => {
  ...
  test('should display "Add from Catalog" button in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should display aliases table in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should open alias catalog in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should display catalog items in stream report modal', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should have "Add New Alias" button in stream report catalog', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should open create alias dialog in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should display steps in create alias dialog for stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should have form fields in create alias dialog for stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should support selecting aliases from catalog in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should display alias details in stream report table', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should have edit and delete actions for stream report aliases', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should close catalog modal in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
...
test.describe('Stream Report Editor - Alias Operations', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`/tableau/stream-report-editor/${MOCK_STREAM_REPORT_ID}`);
    await page.waitForLoadState('networkidle');
  });

  test('should persist aliases when stream report is saved', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should display alias paths correctly in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should display mapper class information in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
  ...
  test('should support bulk operations on aliases in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
    ...
    if (await modelTab.count() > 0) {
      ...
      const checkboxes = page.locator('.ant-table input[type="checkbox"]');
      const exists = await checkboxes.count();
      expect(exists).toBeGreaterThanOrEqual(0);
    }
  });

  test('should validate alias configuration in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');
    ...
    if (await modelTab.count() > 0) {
      ...
      // Validation should occur when saving
      const saveButton = page.locator('button:has-text("Save")');
      const exists = await saveButton.count();
      expect(exists).toBeGreaterThanOrEqual(0);
    }
...
test.describe('Stream Report Editor - Alias Integration with Range', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`/tableau/stream-report-editor/${MOCK_STREAM_REPORT_ID}`);
    await page.waitForLoadState('networkidle');
  });

  test('should work with Range tab in stream reports', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');

    // Stream reports have a unique Range tab
    const rangeTab = page.locator('.ant-tabs-tab:has-text("Range")');
    const exists = await rangeTab.count();
    expect(exists).toBeGreaterThanOrEqual(0);
  });

  test('should allow aliases to be used in stream report configuration', async ({ page }) => {
    await page.waitForSelector('.ant-tabs');

    // Navigate through tabs to ensure aliases work with stream-specific features
    const tabs = ['Model', 'Columns', 'Filter', 'Range'];
    ...
    for (const tabName of tabs) {
      ...
      if (await tab.count() > 0) {
        ...
        const isActive = await activeTab.count();
