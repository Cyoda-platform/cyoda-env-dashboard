...
export const TransitionStateMachineTable: React.FC<TransitionStateMachineTableProps> = ({
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    const saved = storage.get('transitionStateMachineTransition:columnWidths', {});
    const defaultWidths = { type: 250, transactionId: 320, state: 200, message: 300 };
    return saved && Object.keys(saved).length > 0 ? saved : defaultWidths;
  });

  useEffect(() => {
    if (Object.keys(columnWidths).length > 0) {
      storage.set('transitionStateMachineTransition:columnWidths', columnWidths);
    }
  ...
  const columns: ColumnsType<StateMachineEvent> = useMemo(() => [
    ...
    },
    {
      ...
      render: (transactionId: string) => (
        <Link to={`/nodes/${name}/transaction/${transactionId}`}>
          {transactionId}
        </Link>
      ),
    },
  ...
  return (
    <Card className="transition-state-machine-table-transition">
      <Table
        columns={columns}
        dataSource={stateMachineEvents}
        rowKey={(record) => record.event.transactionId}
        bordered
        pagination={{
